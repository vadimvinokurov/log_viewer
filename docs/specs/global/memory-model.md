...
**Memory Optimization:** See [log-entry-optimization.md](../features/log-entry-optimization.md) for optimization strategies including string interning and field removal.

### §3.2 LogDocument (Full Load)
...
- Lifetime: File session (cleared on file close)

**Resource Management:**
- All lines loaded into memory at file open
- No file handle kept open after loading
- Categories extracted during load
...
**Strategy:** Full load for all file sizes.

| File Size | Line Count | Memory Usage | Notes |
|-----------|------------|--------------|-------|
| Any size | Any count | ~200-500 bytes per line | All entries loaded into memory |

**Memory Estimation:**
- Small file (< 100 MB): < 500 MB RAM
- Medium file (100 MB - 1 GB): 500 MB - 5 GB RAM
- Large file (> 1 GB): > 5 GB RAM

**Trade-offs:**
- ✅ Simple implementation
- ✅ Fast filtering and search (all data in memory)
- ✅ No file handle management during session
- ⚠️ Requires sufficient RAM for large files
- ⚠️ Initial load time proportional to file size

**User Guidance:**
- Application should warn if file size exceeds available RAM
- Progress dialog shown during file loading
...
```python
# Clear entries on file close
self._all_entries.clear()
self._all_entries = []
self._document = None
```

**Note:** With full load strategy, file handles are closed immediately after loading. No persistent file handle management required.
...
✅ Clear entries list on file close (`self._all_entries.clear()`)
...
❌ Keep references to entries after file close
...
|---------|------|---------|
| 1.2 | 2026-03-18 | Simplified file loading: full load only, removed lazy load and stream+index |
