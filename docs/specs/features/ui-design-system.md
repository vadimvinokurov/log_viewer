...
**Note:** Complete color system documentation is defined in [color-palette.md](../global/color-palette.md), including:
- Base colors (Gray scale, Accent colors)
- Palette colors (Color families with multiple shades)
- Semantic colors (UI element colors)
- Log level colors (Text, Icon, Statistics)
- Process identification colors
- LogViewer-specific colors
- Accessibility contrast ratios
- Migration guide from legacy constants

This section provides a summary of the most commonly used colors. For complete documentation, see [color-palette.md](../global/color-palette.md).
...
**Note:** Log entry background colors are defined in [table-unified-styles.md](table-unified-styles.md) §4.2.4.
...
The application uses Qt's system default fonts for native look-and-feel on all platforms. Font families and sizes are determined dynamically by Qt, not hardcoded.

**Primary Font (UI Elements):**
- Uses `QApplication.font()` - Qt's system default UI font
- macOS: SF Pro Text
- Windows: Segoe UI
- Linux: System sans-serif font

**Monospace Font (Code/Logs):**
- Uses `QFontDatabase.systemFont(QFontDatabase.SystemFont.FixedFont)`
- macOS: Menlo
- Windows: Consolas
- Linux: System monospace font

**Implementation:** [`Typography`](../../src/constants/typography.py) class

```python
from src.constants.typography import Typography

# Get font family for QSS
font_family = Typography.PRIMARY   # Returns '"SF Pro Text"' on macOS
font_family = Typography.MONOSPACE  # Returns '"Menlo"' on macOS

# Get font size (from system settings)
font_size = Typography.BODY_SIZE  # Returns system default size in points

# Get QFont instances
ui_font = Typography.UI_FONT      # QFont for UI elements
log_font = Typography.LOG_FONT    # QFont for log entries
```
...
|---------|------|--------|-------------|-------|
| Body | System default | Regular (400) | 1.4 | Default text, labels |
| Body Bold | System default | Bold (700) | 1.4 | Emphasized text, counters |
| Header | System default | Bold (700) | 1.3 | Dialog titles |
| Small | System default | Regular (400) | 1.3 | Tooltips, hints |
| Table Header | System default | Regular (400) | 1.0 | Column headers |
| Log Entry | System default | Regular (400) | 1.0 | Log table rows |

**Note:** All font sizes are determined by system settings. The application respects user preferences for font size in macOS System Preferences and Windows Settings.
...
|----------|-------|-------------|
| Minimum Width | None | No minimum window width restriction |
| Minimum Height | None | No minimum window height restriction |

**Implementation:** Window size is unrestricted. The window opens at the system's default size and can be resized to any dimension by the user.

**Rationale:** Users should have full control over window dimensions to accommodate different screen sizes, multi-monitor setups, and personal preferences. The UI components are designed to handle small window sizes gracefully through scrolling and responsive layouts.
...
#### §3.3.3 Component Dimensions

**Note:** Table dimensions are defined in [table-unified-styles.md](table-unified-styles.md) §3.
...
│                           │                                                 │
│  ┌─────┬──────────┬────┬───┐│  ┌─────────────────────────────────────────────┐│
│  │Time │Category  │Type│Msg││  │ [Categories | Filters | Highlights]        ││
│  ├─────┼──────────┼────┼───┤│  ├─────────────────────────────────────────────┤│
│  │...  │...       │... │...││  │ 🔍 [Search categories...]                  ││
│  │...  │...       │... │...││  ├─────────────────────────────────────────────┤│
│  │...  │...       │... │...││  │ ☑ Category1                                ││
...
│  filename.log  │  👁️  │                    │ [⛔ 0] [🛑 0] [⚠️ 0] [ℹ️ 0] [🟪 0] [🟩 0] │
...
**Note:** Complete table styling specification is defined in [table-unified-styles.md](table-unified-styles.md), including:
- Typography for tables
- Spacing and dimensions
- Background colors (header, rows, log levels)
- Borders and grid lines
- Alignment
- States and interactions
- Complete QSS specification

For column alignment specifics, see [table-column-alignment.md](table-column-alignment.md).

For text overflow behavior, see [table-cell-text-overflow.md](table-cell-text-overflow.md).

### §4.4 Tree View

**Note:** Complete tree view styling specification is defined in [category-panel-styles.md](category-panel-styles.md) and [category-tree-row-unification.md](category-tree-row-unification.md), including:
- Tree item states (default, hover, selected)
- Unicode checkbox rendering (U+2610, U+2612, U+25A3)
- Branch indicators
- Row height unification with table

**Summary:**

| Element | Background | Text | Notes |
|---------|------------|------|-------|
| Tree Background | `#F5F5F5` | `#333333` | Panel background |
| Selected Item | `#DCEBF7` | `#000000` | Selection highlight |
| Hover Item | `#E8E8E8` | `#333333` | Hover feedback |

**Checkbox Characters (Custom Drawn):**
...
**Key Dimensions:**
- Item Padding: 0 (unified with table rows)
- Indentation: 10px per level (reduced from Qt default 16px for compact layout)
...
|-------|------------|--------|------|
| Active | Level color (light) | Level color (1px) | Level color (dark) |
| Inactive | `#F5F5F5` | Level color (2px) | `#999999` |
| Hover | Level color (light) | Level color (1px) | Level color (dark) |

**Dimensions:**
- Layout Margins: 4px left/right, 2px top/bottom
- Icon Width: 16px
- Border Radius: 3px
- Spacing: 4px (between icon and count)

**Implementation:** [`CounterWidget`](../../src/views/components/counter_widget.py)
...
**Note:** Counter colors are defined in [`StatsColors`](../../src/constants/colors.py) class.
...
class LogColors:
    ...
    Ref: docs/specs/features/table-unified-styles.md §4.2.4
    """
    CRITICAL: str = "#FF6B6B"
    ERROR: str = "#FF8C8C"
    WARNING: str = "#FFD93D"
    MSG: str = "#B6B6B6"  # Same as default row background
    DEBUG: str = "#B6B6B6"  # Same as default row background
    TRACE: str = "#B6B6B6"  # Same as default row background
...
**Organization:**
```python
# Table dimensions - computed from font metrics
TABLE_ROW_HEIGHT: int  # Computed from Typography.TABLE_ROW_HEIGHT
TABLE_HEADER_HEIGHT: int  # Computed from Typography.TABLE_HEADER_HEIGHT
TABLE_CELL_HEIGHT: int  # Computed from Typography.TABLE_CELL_HEIGHT
...
**Note:** Table dimensions are computed dynamically from font metrics via the `Typography` class. See [`src/constants/typography.py`](../../src/constants/typography.py).

### §7.4 Typography Constants

**File:** [`src/constants/typography.py`](../../src/constants/typography.py)

**Organization:**
```python
class Typography:
    """Simplified typography constants using Qt system fonts."""
    
    # System fonts (QFont instances) - lazy initialization
    UI_FONT: QFont  # System default UI font
    LOG_FONT: QFont  # System monospace font
    
    # Font family strings (for QSS stylesheets)
    @classproperty
    def PRIMARY(cls) -> str:
        """UI font family as string for QSS."""
    
    @classproperty
    def MONOSPACE(cls) -> str:
        """Monospace font family as string for QSS."""
    
    # Font size (from system)
    @classproperty
    def BODY_SIZE(cls) -> int:
        """System default font size in points."""
    ...
    def TABLE_ROW_HEIGHT(cls) -> int:
        """Row height from font metrics + padding."""
    
    @classproperty
    def TABLE_HEADER_HEIGHT(cls) -> int:
        """Header height from font metrics + padding."""
    
    @classproperty
    def TABLE_CELL_HEIGHT(cls) -> int:
        """Unified cell height for both header and rows."""
```
...
- **Color Palette:** [color-palette.md](../global/color-palette.md) - Complete color system documentation
...
|---------|------|---------|
| 1.13 | 2026-03-19 | Added reference to comprehensive color-palette.md specification (§2.1, §9) |
| 1.12 | 2026-03-19 | Removed minimum window size restrictions (§3.3.1) - users can now resize window to any dimension |
| 1.11 | 2026-03-18 | Removed ToggleStrip/CollapsiblePanel from SearchToolbar (panel toggle now via status bar button), updated layout diagram, removed §4.8 Collapsible Panel section, removed §6.2.1 Toggle Strip Arrow Rotation animation |
| 1.10 | 2026-03-16 | Updated tree indentation from 14px to 10px for more compact layout |
| 1.9 | 2026-03-16 | Changed checkbox design to dark gray (#666666) minimalist flat style |
| 1.8 | 2026-03-16 | Changed checkbox from Unicode characters to custom drawn checkboxes matching application style |
| 1.6 | 2026-03-16 | Consolidated tree view styles: §4.4 now references category-panel-styles.md and category-tree-row-unification.md instead of duplicating content |
| 1.5 | 2026-03-15 | Major update: Replaced hardcoded font sizes with Qt system font approach (§2.2), updated window dimensions to match implementation (§3.3.1), corrected counter widget padding specification (§4.6.1), clarified toggle strip hover color discrepancy (§4.8.1), updated implementation guidelines (§7) with Typography class, LogColors.MSG/DEBUG/TRACE now use #B6B6B6, added TABLE_ROW_BACKGROUND constant |
| 1.4 | 2026-03-15 | Refactored: removed table-specific content (moved to table-unified-styles.md), removed §2.1.5 Log Entry Background Colors (moved to table-unified-styles.md §4.2.4), removed §3.3.3 Table Dimensions (moved to table-unified-styles.md §3), replaced §4.3 Tables with cross-references, added cross-references to table specs |
