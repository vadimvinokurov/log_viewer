...
class MainController(QObject):
    ...
    def _connect_signals(self) -> None:
        ...
        # Ref: docs/specs/features/highlight-panel.md §7.3
    ...
    def _setup_file_controller(self) -> None:
        ...
        # File controller connections removed - no file watching
        pass
    ...
    def open_file(self, filepath: str) -> None:
        ...
        # Per docs/specs/features/category-checkbox-behavior.md §5.2
    ...
    def _on_open_file_requested(self) -> None:
        ...
        try:
            if main_script.suffix == '.py':
                ...
                subprocess.Popen([sys.executable, str(main_script), str(pending_filepath)])
            else:
                # Running as executable
                subprocess.Popen([str(main_script), str(pending_filepath)])
        except OSError as e:
    ...
    def _on_index_complete(self, filepath: str) -> None:
        ...
        # Load all entries from document
        # Ref: docs/specs/features/file-management.md §7.1 (Open File Sequence)
        # Ref: docs/specs/global/memory-model.md §8.1 (Memory Release)
        self._all_entries = self._document.get_all_entries()
    ...
    def close_file(self) -> None:
        ...
        // Thread: Must stop IndexWorker before destroying document
        // Memory: Must wait for worker to finish before clearing
        // CRITICAL: Do NOT clear category_panel - it causes Qt paint crash
        //           The new file's set_categories() will replace data safely
        """
        # Stop the index worker if running
        # This must be done BEFORE clearing the document to prevent
        # Qt paint events from accessing destroyed model data
        if self._index_worker is not None:
            if self._index_worker.isRunning():
        ...
        # Clear UI - log table only (NOT category panel)
        # Category panel will be replaced when new file loads
        # Clearing category panel causes Qt paint crash with destroyed model
        self._window.get_log_table().clear()
        # DO NOT clear category panel - it causes crash
        # self._window.get_category_panel().clear()  # REMOVED: causes crash
    ...
    def _save_category_states(self) -> None:
        ...
        Per docs/specs/features/category-checkbox-behavior.md §5.2:
        State is saved when user switches to a different log file.
        """
        if self._document is None:
            return
    ...
    def _restore_category_states(self) -> None:
        ...
        Per docs/specs/features/category-checkbox-behavior.md §5.3:
        - If file was previously viewed, restore its state
        - If new file, all categories default to checked
    ...
    def _apply_filters(self) -> None:
        """Apply current filters to entries with selection and viewport preservation.
        ...
        // Master: docs/SPEC.md §1
        // Thread: Main thread only (per docs/specs/global/threading.md §8.1)
        // Perf: Selection capture O(s), restore O(m), viewport O(1)/O(m), within filter budget
        // Memory: SelectionState and ViewportState are temporary, GC'd after method returns
        """
        ...
        # Step 5: Combine filters with AND logic
        ...
        # Step 6.5: Set document reference for lazy loading raw lines
        # Ref: docs/specs/features/log-entry-optimization.md §4.3
        self._window.get_log_table().set_document(self._document)

        # Step 7: Set entries and restore selection
        # Ref: docs/specs/features/selection-preservation.md §5.2
        # Ref: docs/specs/features/selection-preservation.md §7.4 (skip_scroll for viewport)
        # Skip scrolling if viewport state exists - viewport restoration will handle positioning
        self._window.get_log_table().set_entries(display_entries)
        self._window.get_log_table().restore_selection(
            selection_state,
        ...
        # Ref: docs/specs/features/selection-preservation.md §7.4
        if viewport_state:
            self._window.get_log_table().restore_viewport_position(viewport_state)
    ...
    # === Highlight Pattern Signal Handlers ===
    # Ref: docs/specs/features/highlight-panel.md §7.4
