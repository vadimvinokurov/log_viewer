...
class HighlightDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option, index: QModelIndex) -> None:
        ...
        if option.state & QStyle.State_Selected:
            painter.fillRect(option.rect, QColor(UIColors.BACKGROUND_SELECTED))
        else:
            # Draw default background
            painter.fillRect(option.rect, option.palette.base())
        
        # Get foreground color from model (log level text color)
        # Ref: docs/specs/features/log-level-text-color.md §4.2
        fg_color = index.data(Qt.ForegroundRole)
        
        # When selected, use the specified text color instead of log level color
        # Ref: docs/style_example §4.1.3, docs/specs/global/color-palette.md §10.2.1
        if option.state & QStyle.State_Selected:
            fg_color = QColor(UIColors.TEXT_SELECTED)
        ...
        if self._highlight_engine and isinstance(text, str):
            ...
            if highlights:
                # Build highlighted text with foreground color
                # Ref: docs/specs/features/log-level-text-color.md §6.3
                cursor_pos = 0
                result = ""
                
                # Get foreground color for CSS
                fg_style = f"color: {fg_color.name()};" if fg_color else ""
                
                for hl in highlights:
                    # Add text before highlight (with log level color)
                    if hl.start > cursor_pos:
                        if fg_style:
                            result += f'<span style="{fg_style}">{text[cursor_pos:hl.start]}</span>'
                        else:
                            result += text[cursor_pos:hl.start]
                    # Add highlighted text (user highlight as background)
                    highlighted_text = text[hl.start:hl.end]
                    # User highlight background takes precedence, but keep text color
                    if fg_style:
                        result += f'<span style="background-color: {hl.color.name()}; {fg_style}">{highlighted_text}</span>'
                    else:
                ...
                if cursor_pos < len(text):
                    if fg_style:
                        result += f'<span style="{fg_style}">{text[cursor_pos:]}</span>'
                    else:
                        result += text[cursor_pos:]
                
                doc.setHtml(result)
            else:
                # No highlights, but still need to apply foreground color
                if fg_color:
                    doc.setHtml(f'<span style="color: {fg_color.name()};">{text}</span>')
                else:
                    doc.setPlainText(text)
        else:
            # No highlight engine, apply foreground color directly
            if fg_color:
                doc.setHtml(f'<span style="color: {fg_color.name()};">{text}</span>')
            else:
                doc.setPlainText(text)
