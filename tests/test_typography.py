...
class TestTypography:
    ...
    def test_table_cell_height_derived(self, qapp: QApplication) -> None:
        """Cell height should be derived from LOG_FONT metrics.
        ...
        metrics = QFontMetrics(Typography.LOG_FONT)
        expected_height = metrics.height() + 2
        assert Typography.TABLE_CELL_HEIGHT == expected_height
    
    def test_table_cell_height_matches_row_height(self, qapp: QApplication) -> None:
        """TABLE_CELL_HEIGHT should match TABLE_ROW_HEIGHT (both use LOG_FONT).
        
        Ref: docs/specs/features/table-unified-styles.md §3.2
        """
        assert Typography.TABLE_CELL_HEIGHT == Typography.TABLE_ROW_HEIGHT
...
class TestDimensionsIntegration:
    ...
    def test_get_table_cell_height(self, qapp: QApplication) -> None:
        """get_table_cell_height should return unified cell height.
        
        Ref: docs/specs/features/table-unified-styles.md §3.2
        """
        from PySide6.QtGui import QFontMetrics
        from src.constants.dimensions import get_table_cell_height
        
        # get_table_cell_height should match Typography.TABLE_CELL_HEIGHT
        metrics = QFontMetrics(Typography.LOG_FONT)
        expected_height = metrics.height() + 2
        assert get_table_cell_height() == expected_height
    
    def test_get_table_cell_height_matches_row_height(self, qapp: QApplication) -> None:
        """get_table_cell_height should match get_table_row_height for backward compatibility.
        
        Ref: docs/specs/features/table-unified-styles.md §3.2
        """
        from src.constants.dimensions import get_table_cell_height, get_table_row_height
        
        assert get_table_cell_height() == get_table_row_height()